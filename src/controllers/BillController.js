...
const controller = {
    ...
    getSucceedBill: async (req,res) =>{        
        const {query} = req
        const {isSuccess,status,bills,message} = await services.getSucceedBill(query)
        if(isSuccess){
            return res.status(status).json({isSuccess,bills})
        }
        return res.status(status).json({isSuccess,message})
    ...
    getPendingCancelBill: async (req,res) =>{
        const {query} = req
        const {isSuccess,status,bills,message} = await services.getPendingCancelBill(query)
        if(isSuccess){
            return res.status(status).json({isSuccess,bills})
        }
        return res.status(status).json({isSuccess,message})
